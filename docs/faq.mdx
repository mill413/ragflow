...
For a locally deployed RAGFlow: the total file size limit per upload is 2GB, with a batch upload limit of 50 files. There is no cap on the total number of files per account. To update this 2GB file size limit:

- In **docker/.env**, adjust `MAX_CONTENT_LENGTH=2147483648` as needed. The default value represents 2 GiB in bytes.
...
2. In the **.env** file or from the **Model providers** page in the UI, configure RAGFlow as a remote client to MinerU:
   ...
   - `MINERU_DELETE_OUTPUT`: Whether to delete temporary output when a temporary directory is used:
     - `1`: Delete.
     - `0`: Retain.
   - `MINERU_HEALTHCHECK_TIMEOUT`: (optional) Timeout in seconds for checking the MinerU API's `openapi.json` endpoint. Defaults to `30`.
...
| `MINERU_HEALTHCHECK_TIMEOUT` | Timeout for the MinerU OpenAPI availability check, in seconds | `30` | `MINERU_HEALTHCHECK_TIMEOUT=60` |
