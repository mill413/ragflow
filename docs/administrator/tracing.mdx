...
RAGFlow stores the credentials _per tenant_. Configure them through the authenticated HTTP API:
...
Use `PUT` to replace the saved configuration, `GET` to inspect it, and `DELETE`
to remove it. All four operations use `/api/v1/langfuse/api-key` and require an
authenticated RAGFlow account.
