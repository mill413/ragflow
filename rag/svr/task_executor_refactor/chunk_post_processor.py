...
from api.db.joint_services.tenant_model_service import resolve_model_config_with_fallback
...
async def extract_keywords(docs: List[Dict], ctx: TaskContext) -> None:
    ...
    chat_model_config = resolve_model_config_with_fallback(ctx.tenant_id, LLMType.CHAT, ctx.llm_id, ctx.tenant_llm_id)
...
async def apply_tags(docs: List[Dict], ctx: TaskContext) -> None:
    ...
    chat_model_config = resolve_model_config_with_fallback(tenant_id, LLMType.CHAT, ctx.llm_id, ctx.tenant_llm_id)
    with LLMBundle(ctx.tenant_id, chat_model_config, lang=ctx.language) as chat_model:
        docs_to_tag = []
        for doc in docs:
            if ctx.has_canceled_func(ctx.id):
...
def _resolve_ingestion_chat_llm_id(ctx) -> str:
    """Pick the chat model configured by the document ingestion pipeline."""
...
async def run_document_structure_compile(handler, embedding_model: LLMBundle) -> None:
    ...
    try:
        cfg = resolve_model_config_with_fallback(ctx.tenant_id, LLMType.CHAT, chat_llm_id, ctx.tenant_llm_id)
